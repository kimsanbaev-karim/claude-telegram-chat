...
1. Скилл `/connect-telegram` заставляет агента создать тему в нужном чате и включить `Monitor` — ленту, в которую бот кладёт ему ваши сообщения.
2. Бот, отдав сообщение агенту, ставит на него 👀 — значит агент его увидел.
3. Агент читает ленту без очереди: всё работает так, будто вы написали прямо в терминал.
4. Отвечает он реплаем на ваше сообщение, так что видно, на что именно.

Что это даёт.

**Терминал больше не нужен.** Вся работа идёт с телефона: агенты шлют картинки, видео и markdown-файлы, вы им — тоже.

**Никакого мусора в переписке.** Ни диффов, ни вызовов инструментов, ни служебного шума — только то, что агент отвечает вам, и то, что вы пишете ему.

**Реплай вместо контекста.** Можно ответить на старое сообщение и тем самым сослаться на него, не пересказывая.

**Голосовые вместо печати.** Не нужно возиться с зажатым пробелом, как в терминале: записали голосовое — бот сам расшифровал и отдал агенту текстом. Распознавание локальное, faster-whisper на мультиязычной модели large: три минуты речи разбираются спокойно и без ошибок.
