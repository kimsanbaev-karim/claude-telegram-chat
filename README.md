...
<tr>
<td colspan="2"><img src="docs/topics-desktop.png" alt="Десктоп: слева список тем группы, справа переписка с агентом внутри одной темы"></td>
</tr>
</table>

Слева — тема с телефона: ролик разобран на кадры, голосовые распознаны, 👀 означают «прочитал, работаю». Справа — ответ цитатой на конкретное сообщение и вопрос кнопками, когда агенту нужно решение. Снизу тот же канал с компьютера: слева список тем, где каждая строка — отдельная запущенная сессия, справа разговор внутри одной из них.

## Воркфлоу

Работа идёт из чата Telegram, а не из терминала: агенты присылают картинки, видео и markdown-файлы, вы им — тоже. Реплаем можно ответить и на старое сообщение, сославшись на него без пересказа.

1. Скилл `/connect-telegram` заставляет агента создать через MCP тему в нужном чате и включить `Monitor`, в который телеграм-бот будет класть ему сообщения.
2. Отдав сообщение агенту, бот ставит 👀 на ваше сообщение — значит агент его увидел.
3. Агент читает свой `Monitor` без очереди, так что всё работает так, будто вы написали прямо в терминал.
4. Агент отвечает в Telegram, делая reply на ваше сообщение.
5. Вы видите сообщения агента без мусора в виде диффов от редактирования файлов, вызовов инструментов и прочего — только то, что агент отвечает вам, и то, что вы пишете ему.
6. Распознавание голоса идёт через [faster-whisper](https://github.com/kimsanbaev-karim/whisper-speech-service) на мультиязычной модели large. Увидев голосовое, бот сам расшифровывает его и отсылает агенту. Происходит быстро: голосовые на три минуты распознаются спокойно, ошибок расшифровки замечено не было.
7. Перезапуск сессии ничего не ломает: пока агента нет, бот копит сообщения в ленте и очереди непрочитанного, а подключившись, он забирает их все сразу и отвечает по каждому.
...
Нужен Python 3.12+ и Claude Code. Для видео — `ffmpeg` в PATH, для голосовых — любой локальный whisper, например [whisper-speech-service](https://github.com/kimsanbaev-karim/whisper-speech-service).
...
- **Ничего не теряется при перезапуске.** Бот складывает сообщения в ленту и очередь непрочитанного, пока агента нет. Подключившись, он забирает их все сразу и отвечает по каждому — так что перезапустить сессию можно спокойно.
- **Вопросы кнопками** — `AskUserQuestion` перехватывается хуком и уезжает в тему; свободный текст тоже считается ответом.

## Ограничения

- **Служебные команды через канал не работают.** Из Telegram всё приходит агенту обычным текстом: вызов скилла он поймёт, а команды самого Claude Code — нет, их по-прежнему набирают в терминале.
- **Список тем боту недоступен.** В Bot API нет метода получить темы, поэтому существующую тему ищет пользовательский аккаунт, а создаёт её бот; id темы после создания хранится в реестре `threads.json` — потерять его значит завести дубль темы.
